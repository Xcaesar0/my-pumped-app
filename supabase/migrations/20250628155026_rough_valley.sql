...
-- Initialize user tasks function (create this BEFORE the trigger function)
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    RAISE WARNING 'Error initializing user tasks: %', SQLERRM;
END;
$$;

-- User creation handler (create this AFTER initialize_user_tasks)
...
-- Trigger function for initializing user tasks (separate from handle_new_user)
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Error in trigger_initialize_user_tasks: %', SQLERRM;
    RETURN NEW;
END;
$$;
...
-- User creation trigger (only handles referral code generation)
CREATE TRIGGER on_user_created 
  BEFORE INSERT ON users 
  FOR EACH ROW 
  EXECUTE FUNCTION handle_new_user();

-- Initialize tasks trigger (separate trigger for task initialization)
...
GRANT EXECUTE ON FUNCTION trigger_initialize_user_tasks() TO public;
