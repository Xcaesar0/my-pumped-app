...
BEGIN
  ...
  DROP POLICY IF EXISTS "   Authenticated users can insert their own social connections" ON social_connections;
  DROP POLICY IF EXISTS "Authenticated users can insert their own social connections" ON social_connections;
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Users can read referral rewards" ON point_awards;
  DROP POLICY IF EXISTS "System can insert referral rewards" ON point_awards;
  ...
  DROP POLICY IF EXISTS "Allow authenticated read access" ON admin_tasks;
  ...
  DROP POLICY IF EXISTS "Users can select their own submissions" ON user_task_submissions;
  ...
  DROP POLICY IF EXISTS "Anyone can insert X task completions" ON x_task_completions;
  DROP POLICY IF EXISTS "Users can view their own X task completions" ON x_task_completions;
EXCEPTION
  WHEN OTHERS THEN NULL;
END $$;
...
DROP TRIGGER IF EXISTS trigger_chain_continuation ON referral_tracking CASCADE;
DROP TRIGGER IF EXISTS trg_ensure_user_exists ON social_connections CASCADE;
DROP TRIGGER IF EXISTS trigger_social_connection_points ON social_connections CASCADE;
...
DROP FUNCTION IF EXISTS trigger_chain_continuation() CASCADE;
DROP FUNCTION IF EXISTS trigger_social_connection_points() CASCADE;
...
-- Insert default tasks with proper conflict handling
...
-- =============================================
-- STEP 9: Ensure all users have referral codes
-- =============================================
