/*
  ...
  1. New Tables
    - `point_events` - Track all point-earning events
    - `referral_chains` - Track referral relationships and their status
    - `social_account_connections` - Track when users connect social accounts
    - `point_rules` - Configurable point values for different actions

  2. Point System Logic
    - Referrer earns: +25 (code entry) + 10 (first social) + 15 (second social) + 50 (chain continuation) = 100 max
    - Referee earns: +25 (code entry) + 25 (Twitter) + 25 (Telegram) + 25 (referring someone) = 100 max

  3. Event Tracking
    - All point awards are tracked with reasons and references
    - Referral chains track completion status
    - Social connections trigger point awards automatically
*/
...
CREATE INDEX idx_referral_chains_referrer_id ON referral_chains(referrer_id);
CREATE INDEX idx_referral_chains_referee_id ON referral_chains(referee_id);
CREATE INDEX idx_referral_chains_status ON referral_chains(status);

CREATE INDEX idx_social_connections_user_id ON social_account_connections(user_id);
CREATE INDEX idx_social_connections_platform ON social_account_connections(platform);
