...
-- But first remove any duplicates that might exist
DO $$
BEGIN
  -- Delete duplicates keeping only the oldest record
  DELETE FROM x_task_completions a
  USING (
    SELECT user_id, task_title, MIN(id) as min_id
    FROM x_task_completions
    GROUP BY user_id, task_title
    HAVING COUNT(*) > 1
  ) b
  WHERE a.user_id = b.user_id 
    AND a.task_title = b.task_title 
    AND a.id != b.min_id;
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Could not add unique constraint to x_task_completions: %', SQLERRM;
END $$;
