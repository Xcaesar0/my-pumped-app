/*
  # Consolidated Database Migration - Fixed Version
  
  This migration consolidates all the necessary changes for the referral system,
  social connections, task management, and point tracking system.
  
  It safely handles existing database objects by dropping and recreating them.
*/
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Allow public to update points" ON users;
  DROP POLICY IF EXISTS "Public can insert user data" ON users;
...
-- Drop all existing policies for social_connections table
DO $$
BEGIN
  DROP POLICY IF EXISTS "Public can insert social connections" ON social_connections;
  DROP POLICY IF EXISTS "Public can read social connections" ON social_connections;
  DROP POLICY IF EXISTS "Public can update social connections" ON social_connections;
  DROP POLICY IF EXISTS "Public can delete social connections" ON social_connections;
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Public can read user notifications" ON user_notifications;
  DROP POLICY IF EXISTS "Public can insert user notifications" ON user_notifications;
  DROP POLICY IF EXISTS "Public can update user notifications" ON user_notifications;
  DROP POLICY IF EXISTS "Public can read active admin tasks" ON admin_tasks;
  DROP POLICY IF EXISTS "Public can read user task submissions" ON user_task_submissions;
  DROP POLICY IF EXISTS "Public can create user task submissions" ON user_task_submissions;
  DROP POLICY IF EXISTS "Public can update user task submissions" ON user_task_submissions;
...
DROP TRIGGER IF EXISTS trigger_social_connection_points ON social_connections CASCADE;
...
DROP FUNCTION IF EXISTS process_social_connection_points(uuid, text) CASCADE;
DROP FUNCTION IF EXISTS trigger_social_connection_points() CASCADE;
DROP FUNCTION IF EXISTS process_x_task_completion(text, uuid, text) CASCADE;
...
-- Update platform constraint for social_connections
DO $$
BEGIN
  ALTER TABLE social_connections DROP CONSTRAINT IF EXISTS social_connections_platform_check;
  ALTER TABLE social_connections ADD CONSTRAINT social_connections_platform_check CHECK (platform IN ('telegram', 'x'));
EXCEPTION
  WHEN OTHERS THEN NULL;
END $$;
...
-- Add unique constraint to x_task_completions if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
CREATE POLICY "Allow authenticated read access" ON admin_tasks FOR SELECT TO authenticated USING (true);
CREATE POLICY "Allow public read access to active admin tasks" ON admin_tasks FOR SELECT TO public USING (is_active = true);

-- User task submissions policies
CREATE POLICY "Allow authenticated read access" ON user_task_submissions FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can create their own submissions" ON user_task_submissions FOR INSERT TO public WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can insert their own submissions" ON user_task_submissions FOR INSERT TO public WITH CHECK (EXISTS ( SELECT 1 FROM users u WHERE ((u.wallet_address = (auth.uid())::text) AND (u.id = user_task_submissions.user_id))));
CREATE POLICY "Users can select their own submissions" ON user_task_submissions FOR SELECT TO public USING (EXISTS ( SELECT 1 FROM users u WHERE ((u.wallet_address = (auth.uid())::text) AND (u.id = user_task_submissions.user_id))));
CREATE POLICY "Users can update their own pending submissions" ON user_task_submissions FOR UPDATE TO public USING ((auth.uid() = user_id) AND (status = 'pending'::text));
CREATE POLICY "Users can view their own submissions" ON user_task_submissions FOR SELECT TO public USING (auth.uid() = user_id);

-- X task completions policies
CREATE POLICY "Anyone can insert X task completions" ON x_task_completions FOR INSERT TO public WITH CHECK (true);
CREATE POLICY "Users can view their own X task completions" ON x_task_completions FOR SELECT TO authenticated USING (auth.uid() = user_id);
...
-- =============================================
-- STEP 9: Insert default admin tasks
-- =============================================
...
-- =============================================
-- STEP 11: Ensure all users have referral codes
-- =============================================
