/*
  ...
  2. Updates
    - Update social_connections to allow 'x' platform
    - Add proper indexes for performance
    - Create RLS policies compatible with wallet-based auth

  3. Security
    - Enable RLS on both tables
    - Create policies that work without Supabase auth
...
-- Since we're using wallet-based auth, we'll allow public access with application-level validation
CREATE POLICY "Users can insert their own submissions"
  ON user_task_submissions
  FOR INSERT
  TO public
  WITH CHECK (user_id IN (SELECT id FROM users WHERE id = user_id));
...
CREATE POLICY "Users can update their own pending submissions"
  ...
  FOR UPDATE
  TO public
  USING (user_id IN (SELECT id FROM users WHERE id = user_id) AND status = 'pending')
  WITH CHECK (user_id IN (SELECT id FROM users WHERE id = user_id) AND status = 'pending');
